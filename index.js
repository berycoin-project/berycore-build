 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`berycore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`berycore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'berycore-' + name : 'berycore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/berycore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external berycore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:berycore-lib -o berycore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('berycoin-project', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('berycoin-project', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push berycoin-project $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
